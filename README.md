# VictoryX — tennis coaching

Static marketing site (HTML / CSS / JS, no build step) hosted on GitHub Pages
at `victoryx.shop`.

> Generated from `C:\Users\souha\coaching-sites-factory` (content file `sites/victoryx.mjs`).
> To change the content, edit that file and run `node build.mjs victoryx` — editing the
...
| 🟠 Important | `contact@victoryx.shop` doesn't exist yet: set up free email forwarding in Namecheap (*Domain List → Manage → Redirect Email*). | Namecheap |
...
```
Coaching in tennis, delivered online: video analysis of the player's strokes, one-on-one video sessions and small group sessions covering groundstroke and serve technique, match strategy and tactics, and structured practice planning. Players follow an 8 to 16 week plan with weekly video submissions and written analysis. All on-court practice takes place at the player's own club or court, with their own hitting partner or in-person coach; no on-court instruction is provided. Services are sold as month-to-month subscriptions from $35 to $189 per month, cancellable at any time. No equipment or physical products are sold or shipped. Site: victoryx.shop
```
